...
function VideoNovo() {
    ...
    const validar = () => {
        ...
        Object.entries(campos).forEach(([key, value]) => {
            switch (key) {
                case "titulo":
                    if (!value) novoErrors[key] = "El Título es obligatorio";
                    break;
                case "categoria":
                    if (!value) novoErrors[key] = "La categoría es obligatoria";
                    break;
                case "imagem":
                    if (!value) {
                        novoErrors[key] = "La imagen es obligatoria";
                    } else if (!validarURL(value)) {
                        novoErrors[key] = "La URL de la imagen no es válida";
                    }
                    break;
                case "video":
                    if (!value) {
                        novoErrors[key] = "El video es imprescindible";
                    } else if (!validarURL(value)) {
                        novoErrors[key] = "La URL del vídeo no es válida";
                    }
                    break;
                case "descricao":
                    if (!value) novoErrors[key] = "La descripción es obligatoria";
                    break;
            }
        });
    ...
    const aoEnviar = async (evento) => {
        ...
        if (Object.keys(novoErrors).length === 0) {
            try {
                ...
                if (!response.ok) {
                    throw new Error("Error al crear una nueva tarjeta.");
                }
                ...
                setVideo("");
                setDescricao("");
                mostrarNotificacao("Nueva tarjeta creada con éxito!", "success");
            } catch (error) {
                console.error("Error al crear una nueva tarjeta:", error);
                mostrarNotificacao("Error al crear una nueva tarjeta. Por favor, inténtelo de nuevo.", "error");
            }
        } else {
            ...
            <ContainerVideoNovo>
                ...
                <div>
                    <Form onSubmit={aoEnviar}>
                        <TituloEditar>Crear Card:</TituloEditar>
                        <BoxInputsPrimarios>
                            <BoxInputs>
                                <Label>Título</Label>
                                <Input
                        ...
                        <BoxBotoes>
                            ...
                            <Botao type="button" onClick={limparInputs}>LIMPIAR</Botao>
                        </BoxBotoes>
                    </Form>
                </div>
            </ContainerVideoNovo>
